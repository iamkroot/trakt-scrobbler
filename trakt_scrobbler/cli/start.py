...
@app.command(
    help="Starts the trakt-scrobbler service. If already running, does nothing."
)
def start(
    restart: Annotated[
        bool, typer.Option("--restart", "-r", help="Restart the service.")
    ] = False,
):
    if platform == "darwin":
        if restart:
    ...
    from trakt_scrobbler.cli.status import status
